...
export default function Dashboard() {
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      {/* ── Page Header ── */}
      <div>
        <h1 className="text-xl font-bold text-foreground">Admin Portal Dashboard</h1>
        <p className="text-xs text-muted-foreground">
          Welcome to the RestroPartner management suite. Monitor system statistics and perform administration task.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <IconClock className="size-4 text-primary animate-pulse" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {requestsLoading ? <Skeleton className="h-8 w-12" /> : pendingRequestsCount}
            </div>
            <p className="text-[10px] text-muted-foreground mt-0.5">Waitlisted restaurant partners</p>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {orgsLoading ? <Skeleton className="h-8 w-12" /> : activeOrgsCount}
            </div>
            <p className="text-[10px] text-muted-foreground mt-0.5">Onboarded restaurant entities</p>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {outletsLoading ? <Skeleton className="h-8 w-12" /> : activeOutletsCount}
            </div>
            <p className="text-[10px] text-muted-foreground mt-0.5">Live operating restaurant branches</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
          ...
          <Card className="shadow-sm h-full flex flex-col justify-between">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-sm font-bold">Recent Early Access Signups</CardTitle>
                  <CardDescription className="text-xs">
                    Latest submissions looking for RestroPartner platform onboarding.
                  </CardDescription>
                </div>
                <Link to="/requests">
            ...
            <CardContent className="flex-1">
              {requestsLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-muted-foreground text-center">
                  <IconFileText className="size-8 opacity-20 mb-2" />
                  <p className="text-xs">No restaurant requests registered in waitlist.</p>
                </div>
              ) : (
                <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-4">
          <Card className="shadow-sm">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-bold">Quick Administrative Links</CardTitle>
              <CardDescription className="text-xs">
